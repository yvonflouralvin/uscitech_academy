...
export default function TeacherList () {
    ...
    const load = async (page: number, search?: string) => {
        const url = `/uscitech_academy/teachers/?page=${page}${search !== undefined ? `&search=${search}` : ""}`
        try {
    ...
    <div>
        ...
        {
            results.map((item: Teacher, index) => {
                return <Link href={`/apps/uscitech_academy/teachers/${item.id}`} key={item.id} className="duration-300 flex gap-[2px] text-[13px] text-gray-500 px-[20px] py-[6px] my-[3px] cursor-pointer hover:bg-[rgba(0,0,0,0.02)] border-b border-inherent">
                    <p className="w-[50%]">{item.employee.user.name} {item.employee?.user.last_name} {item.employee?.user.first_name} </p>
                    <p className="w-[25%]">{item.employee.user.email}</p>
                </Link>
            })
        }
    </div>
